import React from 'react';
/*------------------------ SVG for Single Tick ----------------------------*/
export const SingleTick: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      fill="currentColor"
      className="bi bi-check"
      viewBox="0 0 16 16"
      {...props}  
    >
      <path d="M13.854 3.146a.5.5 0 0 0-.708 0L5.5 10.793 2.854 8.146a.5.5 0 1 0-.708.708l3 3a.5.5 0 0 0 .708 0l8-8a.5.5 0 0 0 0-.708z" />
    </svg>
  );
  /*------------------------ SVG for Double Tick ----------------------------*/
export const DoubleTick: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 448 512"
      width="24"
      height="24"
      fill="currentColor"
      {...props}  
    >
      <path d="M342.6 86.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L160 178.7l-57.4-57.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l80 80c12.5 12.5 32.8 12.5 45.3 0l160-160zm96 128c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L160 402.7 54.6 297.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l128 128c12.5 12.5 32.8 12.5 45.3 0l256-256z"/>
    </svg>
  );


